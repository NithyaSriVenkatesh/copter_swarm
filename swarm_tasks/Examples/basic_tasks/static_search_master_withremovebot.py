...
uav1_server_address = ('192.168.1.103', 12002)  #receive from .....rx.py
...
if(file_name=="Medur_"):
	...
	goal_points=[ (179.40550274627913,308.15386473749555), (170.99513572947805,289.35827568680276), (168.86005545367016,278.07166026184615), (196.9510938076667,266.3208209010031), (221.81169795738273,255.76690549382712), (141.36847098185353,246.6297974398802), (159.12810083196678,241.36795466086144), (165.17736445453224,255.9836791630281), (172.83867941495768,274.7264309925827), (221.20009527209496,256.0336190545783), (182.32709847774157,232.5208029351906), (207.48096771410658,224.8346429075841), (211.9462937186256,237.44651100896382), (264.87664269879014,206.67858540983394), (277.8934118507254,235.80815686557324), (298.4555948189716,230.78146926893683), (293.17787532288014,273.5376866780221), (240.41019857324855,302.7874959996908), (188.6930646784506,245.50582780455525), (230.1665459467102,218.5718998451968), (123.06874511896862,243.5163968586071), (139.11279072454997,286.02406938652103), (160.29300426583518,342.7190679601935), (186.27125160742494,330.61502836178624),]
	disperse_multiple_goals=[(126.49449142221913 ,252.22651165464404),( 172.1520264516062 , 327.39164945923085 ),( 196.8655168435012 , 274.1906809896087)]
	agg_goal_point=(4011.961697874066,2949.183646467862) 
	start_multiple_goals=[(3940.83482945579,2919.7068794967186),(3942.2133504105423,2934.311011924758),(3959.4546415666505,2941.2243562221956)]

	multiple_goals=start_multiple_goals
	return_multiple_goals=[(3940.83482945579,2919.7068794967186),(3942.2133504105423,2934.311011924758),(3959.4546415666505,2941.2243562221956)]
	area_covered_percent=40
...
vehicles=[]
pos_array=[]
heartbeat_ip=["192.168.6.151","192.168.6.152","192.168.1.103","192.168.6.154","192.168.6.155","192.168.6.156","192.168.6.157","192.168.6.158","192.168.6.159","192.168.6.160"]
heartbeat_ip_timeout=[0]*10
heartbeat_ip_timeout=[1,30,30,30,30,3,3,3,3,3]
goal_path_csv_array=[]
...
server_address1 = ('192.168.1.105', 12010)
slave_heal_ip=["192.168.1.103"]*num_bots
...
while(1):
	...
	try:            
		...
		if(data==b"disperse") or (disperse_flag):
			...
			d_all_uav_csv_grid_array=[0]*num_bots
			...
			for i,b in enumerate(s.swarm):
				d_all_uav_csv_grid_array[i]=csv_file_paths.get(i)
			print("d_all_uav_csv_grid_array!!!!",d_all_uav_csv_grid_array)	
			for i,z in enumerate(d_all_uav_csv_grid_array):
				goal_lat_lon = read_specific_line(d_all_uav_csv_grid_array[i], 1)
				print("goal_lat_lon",goal_lat_lon,origin,endDistance,goal_lat_lon[0][0])
				x,y = locatePosition.geoToCart (origin, endDistance, [goal_lat_lon[0][0],goal_lat_lon[0][1]])
				disperse_multiple_goals.append((x/2,y/2))
			...
			while True:	
				...
				if(num_bots==10):
					...
					time.sleep(0.03)#verified
				elif(num_bots==5):
					time.sleep(0.025)#verified
				elif(num_bots==4):
					time.sleep(0.025)
				elif(num_bots==3):
					time.sleep(0.02)
				...
				try:
					...
					for i,b in enumerate(s.swarm):
						...
						if(multiple_goals==[]):
							...
							cmd+= disp_field(b,neighbourhood_radius=500)*10
		...
		if(data==b"search") or (search_flag):
			...
			while 1:	
				if(num_bots==10):
					...
					time.sleep(0.03)##0.04 speed 2-3m/s
				elif(num_bots==6):
					time.sleep(0.03)#verified
				elif(num_bots==5):
					time.sleep(0.035)#verified
				elif(num_bots==4):
					time.sleep(0.04)
				elif(num_bots==3):
					time.sleep(0.02)
