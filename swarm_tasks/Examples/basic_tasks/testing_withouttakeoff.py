...
csv_file_path = "/home/dhaksha/Desktop/dce_swarm1_disperse/dce_swarm_nav/swarm_tasks-main/swarm_tasks/Examples/basic_tasks/uav_data_log.csv" 
csv_file_path1 = "/home/dhaksha/Desktop/dce_swarm1_disperse/dce_swarm_nav/swarm_tasks-main/swarm_tasks/Examples/basic_tasks/waypoints.csv" 
nextwaypoint=0
...
try:	
	vehicle9= connect('udpin:192.168.29.125:14559',baud=115200,heartbeat_timeout=3)
...
try:	
	vehicle10= connect('udpin:192.168.29.125:14560',baud=115200,heartbeat_timeout=3)
	...
	try:            
		...
		if(data==b"takeoff"):
			...
			for i, vehicle in enumerate(vehicles):
			    ...
			    if same_alt_flag:
				    thread = threading.Thread(target=arm_and_takeoff, args=(vehicles[i], different_height[i]))
				    print("KKKKKKK")
			    else:
				    print("HHHHHHH")
				    thread = threading.Thread(target=arm_and_takeoff, args=(vehicles[i], same_height))
			    print("thtead",thread)
				...
				while 1:
					...
					if(step==1):											
						for i,b in enumerate(s.swarm):
							...
							with open('/home/dhaksha/Desktop/dce_swarm1_disperse/dce_swarm_nav/swarm_tasks-main/swarm_tasks/Examples/basic_tasks/pos.csv','rt')as text:
						...
						for i,b in enumerate(s.swarm):							
							...
							goal=multiple_goals[goal_table[i]]
							print("goal",goal)
							...
							if(dx<=10 and dy<=10):
								bot_array[i]=1
								goal_table[i]+=1
								print("goal=multiple_goals[goal_table[i]]",goal_table)
								print("bot_array",bot_array)
							...
							if (all_bot_reach_flag==True):								
								#print("Goal_reached!!!!!!!!!!!")
								with open(r'/home/dhaksha/Desktop/dce_swarm1_disperse/dce_swarm_nav/swarm_tasks-main/swarm_tasks/Examples/basic_tasks/pos.csv', 'a') as csvfile:
									fieldnames = ['waypoint']
								...
								while True:
									...
									if(step==1):										
										...
										for i,b in enumerate(s.swarm):
											...
											#print("goal",goal)
											#cmd =cvg.goal_area_cvg(i,b,goal,use_base_control=True, exp_weight_params=[15.0,15.5],disp_weight_params=[30.0,30.0])
											cmd = base_control.base_control(i,b,(239.63579710743477,301.23115906075583))*10
											cmd+= base_control.obstacle_avoidance(i,b,(239.63579710743477,301.23115906075583))*50
				...
				while 1:	
					...
					if(step==1):
						...
						for i,b in enumerate(s.swarm):
							...
							#current_position=[b.x,b.y]
							#goal=bot_paths[goal_table[i]][count[i]]
							print("goal_table[i]",goal_table[i])
							goal=multiple_goals[goal_table[i]]
							print("goal",goal)
							#Base control
							#cmd = base_control.base_control(i,b,goal)
