...
# Bind the socket to the port
graph_server_address = ('192.168.55.174', 12009)  #receive from .....rx.py
...
uav2_server_address = ('192.168.55.174', 12002)  #receive from .....rx.py
...
#uav4_server_address = ('192.168.55.174', 12002)
#uav5 = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
#uav5_server_address = ('192.168.55.174', 12002)
...
if(file_name=="Medur_"):
	...
	start_multiple_goals=[(57.6212021899204, 78.10325083271421),(106.87887371905668, 77.79531213686867),(167.33901982007927, 79.59851216861783),(227.25745287329676, 81.11971011249729)]
	agg_goal_point=(227.25745287329676, 81.11971011249729)
	print("agg_goal_point",agg_goal_point)
...
# Bind the socket to the port
server_address = ('192.168.55.174', 12005)  #receive from .....rx.py

# Bind the socket to the port
remove_bot_server_address = ('192.168.55.174', 12001)  #receive from .....rx.py
...
vehicles=[]
pos_array=[]
heartbeat_ip=["192.168.6.151","192.168.55.174","192.168.6.153","192.168.6.154","192.168.6.155","192.168.6.156","192.168.6.157","192.168.6.158","192.168.6.159","192.168.6.160"]
heartbeat_ip_timeout=[0]*10
heartbeat_ip_timeout=[30,30,30,3,3,3,3,3,3,3]
goal_path_csv_array=[]
...
def vehicle_connection():	
	...
	try:	
		vehicle1= connect('udpin:192.168.55.174:14561',baud=115200, heartbeat_timeout=heartbeat_ip_timeout[0])
	...
	try:		
		vehicle2= connect('udpin:192.168.55.174:14562',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[1])
	...
	try:
		vehicle3= connect('udpin:192.168.55.174:14563',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[2])
	...
	try:		
		vehicle4= connect('udpin:192.168.55.174:14564',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[3])
	...
	try:		
		vehicle5= connect('udpin:192.168.55.174:14555',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[4])
	...
	try:
		vehicle6= connect('udpin:192.168.55.174:14556',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[5])
	...
	try:
		vehicle7= connect('udpin:192.168.55.174:14557',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[6])
	...
	try:
		vehicle8= connect('udpin:192.168.55.174:14558',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[7])
	...
	try:	
		vehicle9= connect('udpin:192.168.55.174:14559',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[8])
	...
	try:	
		vehicle10= connect('udpin:192.168.55.174:14560',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[9])
...
slave_heal_ip=["192.168.55.174"]*num_bots
...
server_address1 = ('192.168.55.174', 12010)
slave_heal_ip=["192.168.55.174"]*num_bots
...
while(1):
	...
	try:            
		...
		if(data.startswith(b"remove_bot")) or (remove_flag):	
				...
				for l in range(0,len(pos_array)):	
					if(int(remove_bot_num)==pos_array[l]):
						...
						print(l)
						break
				remove_bot_index.append(pop_bot_index)
				...
				home_pos_lat_lon.pop(pop_bot_index)				
				removed_grid_path_array_start_val.pop(pop_bot_index)
				checkall_removed_grid_path_array_start_val.pop(pop_bot_index)
				#num_bots-=1
				#print(num_bots)
				print("!!!!!!!!!!!!pop_flag_arr!!!!!!!!!!!",pop_flag_arr,checkall_removed_grid_path_array_start_val,removed_grid_path_array_start_val)
				sent = remove_socket.sendto(str(pop_bot_index).encode(), remove_bot_server_address)
				print("pop index",pop_bot_index)
				uav_home_pos=[]
						...
						for i,b in enumerate(s.swarm):							
							...
							if master_flag:
								...
								msg = (str(robot1_x) + ',' + str(robot1_y) + ',' +
							       ...
							       str(robot10_x) + ',' + str(robot10_y))
					message_count+=1				
					if(message_count%30)==0 and master_flag:				       
		...
		if(data==b"disperse") or (disperse_flag):
			...
			disperse_multiple_goals=[0]*len(pos_array)
			print("cwd",cwd)
			base_dir = os.path.join(cwd, 'searchgrid', file_name)
		    # Create the base directory if it doesn't exist
			if not os.path.exists(base_dir):
				os.makedirs(base_dir)
			
		    # Append CSV file paths to the list
			csv_file_paths = []
			sub_dir=""
			if(remove_bot_flag):
			    remove_bot_index=[]
			...
			for i in range(len(pos_array)):
				print("VVVVVVVV",i,base_dir)
				sub_dir = os.path.join(base_dir, f'{len(pos_array)}drones')
				if not os.path.exists(sub_dir):
					os.makedirs(sub_dir)
			GridPlanning.CreateGridsForSpecifiedAreaAndSpecifiedDrones( 13.389460, 80.233607,len(pos_array),5,100,sub_dir)
			for i in range(1,len(pos_array)+1):
			    csv_file_paths.append(os.path.join(cwd, 'searchgrid', f'{file_name}',f'{len(pos_array)}drones',f'd{i}.csv'))
			print(csv_file_paths)
			for i in range(len(pos_array)):
				print("JJJJ",i)
				goal_lat_lon = read_specific_line(csv_file_paths[i], 1)
				print("goal_lat_lon",goal_lat_lon)
				x,y = locatePosition.geoToCart (origin, endDistance, [goal_lat_lon[0][0],goal_lat_lon[0][1]])
				disperse_multiple_goals[i]=(x/2,y/2)	
				print("disperse_multiple_goals",disperse_multiple_goals)
