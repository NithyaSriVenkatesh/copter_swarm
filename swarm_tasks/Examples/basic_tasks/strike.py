...
uav2_server_address = ('192.168.6.154', 12002)  #receive from .....rx.py
uav3 = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
uav3_server_address = ('192.168.6.155', 12002)
uav4 = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
uav4_server_address = ('192.168.6.160', 12002)
#uav5 = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
#uav5_server_address = ('192.168.6.155', 12002)
...
csv_file_path = "/home/dhaksha/Desktop/meharbaba/swarm_tasks/Examples/basic_tasks/uav_data_log.csv" 
csv_file_path1 = "/home/dhaksha/Desktop/meharbaba/swarm_tasks/Examples/basic_tasks/waypoints.csv" 
nextwaypoint=0
...
#heartbeat_ip_timeout=[3,3,30,30,30,3,3,3,3,30]
goal_path_csv_array=[]
...
def vehicle_collision_moniter_receive():	
		...
		#vehicle_uav_heartbeat_flag=True        	
        	...
        	if index.startswith(b"master"):
        		...
        		m,master_num=decoded_index.split("-")
        		print("m,master_num",m,master_num)
        		msg='NUC 2 master_num '+str(master_num)+' data received'
        		for l in range(2):
...
while True:
	if(uav_home_pos!=[]):
		...
		for l in range(3):
			graph_socket.sendto(str("NUC 2 CONNECTED").encode(), graph_server_address)	
		break
	else:
		pass
#gui = viz.Gui(s)
...
def set_velocity(i,velocity_x, velocity_y, velocity_z,yaw):
        #only let commands through at 10hz
        global vehicles
        vel_update_rate=0.1
        print("velocity_x, velocity_y, velocity_z,yaw",velocity_x, velocity_y, velocity_z,yaw,i,vel_update_rate)
        if(time.time() - 0) > vel_update_rate:
            last_set_velocity = time.time()   
            # create the SET_POSITION_TARGET_LOCAL_NED command
            msg = vehicles[i].message_factory.set_position_target_local_ned_encode(
...
while(1):
	...
	try:            
					...
					while 1:						
						...
						try:
							...
							if data.startswith(b'specific_bot_goal'): 
								...
								if goal_point_num.startswith(b'H'): 
									letter, goal_point_num = goal_point_num[0], int(goal_point_num[1:])
									print(letter, goal_point_num)						
									goal_position = home_pos[int(goal_point_num) - 1]
									print("HOME######## goal_position",goal_position,int(goal_point_num))
								...
								if(specific_bot_goal_flag_array[i]):
									...
									if master_flag:
										...
										if same_alt_flag:
											...
											point1 = LocationGlobalRelative(lat,lon,different_height[i])
										#print("point1",point1)
										vehicles[int(uav)].simple_goto(point1)
										if i==0:
		...
		if(data==b"search") or (search_flag):
			...
			strike_flag_array=[False]*num_bots
			strike_pos_array=[0]*num_bots
			strike_flag=False	
			climb_height=0		
			...
			descent_altitude=0
			alt=[0]*num_bots
			climb_height_flag=True
			guided_target_vel=[]
			yaw_final=0.0
			Targetdistance=0.0
			striked_flag=False
			while 1:	
				#print("kamikaze.camera_width",kamikaze.camera_width,kamikaze.R)
				if(num_bots==10):
					time.sleep(0.005)
				elif(num_bots==9):
					time.sleep(0.008)
				...
				try:
					...
					if(decoded_index.startswith("strike")):
						f,uav,climb_height, goal_lat,goal_lon,descent_altitude = decoded_index.split(",")
						#uav,goalx,goaly=str(index).split(",")
						print('decoded_index',decoded_index)
						print(f,uav,climb_height, goal_lat,goal_lon,descent_altitude,"f,uav,climb_height, goal_lat,goal_lon,descent_altitude")
						print("pos_array",pos_array)
						climb_height=int(climb_height)
						descent_altitude=int(descent_altitude)
					...
					if(f=="strike"):
						...
						strike_goal_bot_num=int(uav)
						print(strike_goal_bot_num,"strike_goal_bot_num")
						goal_position=[float(goal_x/2),float(goal_y/2)]
						print("goal_position",goal_position)
						#strike_goal_bot_num=strike_goal_bot_num-1
						print("strike_goal_bot_num",strike_goal_bot_num)
						strike_flag_array[strike_goal_bot_num]=True
						strike_pos_array[strike_goal_bot_num]=goal_position
						print("strike_flag_array,strike_pos_array",strike_flag_array,strike_pos_array)
					...
					for i,b in enumerate(s.swarm):
						...
						elif(strike_flag) or (strike_flag_array[i]):
							if(strike_flag_array[i]):
								c_lat = vehicles[strike_goal_bot_num].location.global_frame.lat
								c_lon = vehicles[strike_goal_bot_num].location.global_frame.lon	
								print("c_lat,c_lon",c_lat,c_lon,alt,strike_goal_bot_num)
								Targetdistance=kamikaze.gps_distance(c_lat,c_lon,float(goal_lat),float(goal_lon))
								print("Targetdistance",Targetdistance)
								Tbearing=kamikaze.gps_bearing(c_lat,c_lon,float(goal_lat),float(goal_lon))
								print("Targetdistance",Targetdistance,Tbearing)
				
								print("goal_position",i,strike_pos_array[i])
								print("climb_height",climb_height)  
								alt=vehicles[i].location.global_relative_frame.alt 
								print("alt[vehicle]",alt)
								current_position =[b.x*2,b.y*2]
								#print("Drone 1 position",b.x,b.y)
								if strike_flag_array[i] and striked_flag:
									if vehicles[i].location.global_relative_frame.alt  <= 30:
										print("altitude",vehicles[i].location.global_relative_frame.alt)
										strike_flag_array[i]=False
										striked_flag=False
										vehicles[strike_goal_bot_num].mode = VehicleMode("RTL")
										break
								if Targetdistance<250  :
									c_lat = vehicles[i].location.global_frame.lat
									c_lon = vehicles[i].location.global_frame.lon	
									print("c_lat,c_lon",c_lat,c_lon,alt,strike_goal_bot_num)
									distance_meters=250-Targetdistance
									#opposite_bearing = (Tbearing + 180) % 360
									opposite_bearing=0
									print("distance_meters,opposite_bearing",distance_meters,opposite_bearing)
									# Calculate the new latitude and longitude
									heading_rad = math.radians(opposite_bearing)
									print("heading_rad",heading_rad)
									new_lat = c_lat + (distance_meters / 111111.0) * math.cos(heading_rad)
									new_lon = c_lon + (distance_meters / (111111.0 * math.cos(math.radians(c_lat)))) * math.sin(heading_rad)
									#new_lon = c_lon
									print("new_lat,new_lon",new_lat,new_lon)
									vehicles[i].simple_goto(LocationGlobalRelative(new_lat, new_lon, int(climb_height) ))
									while True:
										c_lat = vehicles[i].location.global_frame.lat
										c_lon = vehicles[i].location.global_frame.lon	
										alt=vehicles[i].location.global_relative_frame.alt
										Targetdistance=kamikaze.gps_distance(new_lat,new_lon,float(goal_lat),float(goal_lon))
										#print("Targetdistance",Targetdistance)
										dist=kamikaze.gps_distance(c_lat,c_lon,new_lat,new_lon,)
										print("dist",dist)
										if int(climb_height) - 1.5 <= alt <= int(climb_height) and dist<50:
											break

									#time.sleep(30)
								else:
									climb_height_flag=True
									while climb_height_flag:
										if(index==b"strike_stop"):
											index="data"
											print("STOPPPPPPPPPP!!!!!!!")
											graph_socket.sendto(str("Strike Cancelled").encode(), graph_server_address)				
											strike_flag_array[i]=False
											break
										lat,lon = locatePosition.cartToGeo (origin, endDistance, current_position)
										point1 = LocationGlobalRelative(lat,lon,int(climb_height))
										vehicles[i].simple_goto(point1)
										alt=vehicles[i].location.global_relative_frame.alt
										if int(climb_height) - 1.5 <= alt <= int(climb_height):
											climb_height_flag=False
									
										
									
								while vehicles[i].location.global_relative_frame.alt  > 30:
									if vehicles[i].location.global_relative_frame.alt  <= 30:
										print("altitude",vehicles[i].location.global_relative_frame.alt)
										strike_flag_array[i]=False
										vehicles[strike_goal_bot_num].mode = VehicleMode("RTL")
									...
									if(index==b"strike_stop"):
										...
										strike_flag_array[i]=False
										break
									if(i==strike_goal_bot_num) and  vehicles[i].location.global_relative_frame.alt  > 30:
										guided_target_vel,yaw_final=kamikaze.move_to_target(c_lat,c_lon,float(goal_lat),float(goal_lon),alt)										
										alt=vehicles[i].location.global_relative_frame.alt
										print("alt",alt,strike_goal_bot_num)
										
										set_velocity(strike_goal_bot_num,guided_target_vel[0], guided_target_vel[1], guided_target_vel[2],yaw_final)
										striked_flag=True
								...
								"""	
										...
										if same_alt_flag:
											...
											point1 = LocationGlobalRelative(lat,lon,different_height[i])										
										vehicles[i].simple_goto(point1)								
								b.set_goal(strike_pos_array[i][0],strike_pos_array[i][1])
								cmd = cvg.goal_area_cvg(i, b, strike_pos_array[i])
								print("cmd11111 exceuted")
								current_position=[b.x,b.y]
								dx=abs(strike_pos_array[i][0]-current_position[0])
								dy=abs(strike_pos_array[i][1]-current_position[1])
								print("dx,dy",dx,dy)
								print("strike_flag_array",strike_flag_array)
								print("strike_pos_array",strike_pos_array)
								if(dx<=20 or dy<=20):
									strike_enable=True
									print("strike_enable",strike_enable)
								"""
							else:								
								cmd = cvg.disp_exp_area_cvg(b, use_base_control=True, exp_weight_params=[1.0,1.5],disp_weight_params=[2.0,2.5]) *2
						else:											
						...
						if master_flag:							
							if strike_flag_array[i]:
								...
								if(strike_enable):
									pass
									'''
									value=(goal_position[0]*2,goal_position[1]*2)
									print("value",value,goal_position,goal_x,goal_y)
									if master_flag:
										print("KKKKKKKKKKKKKKKKK")
										lat,lon = locatePosition.cartToGeo (origin, endDistance, value)
										point1 = LocationGlobalRelative(lat,lon,int(descent_altitude))	
										print("descent_altitude",descent_altitude)
										vehicles[i].simple_goto(point1)
									
									for l,vehicle in enumerate(vehicles):
										alt=vehicles[i].location.global_relative_frame.alt
										if int(descent_altitude) - 1.5 <= alt <= int(descent_altitude)+1.5:
											vehicles[i].mode = VehicleMode("RTL")
									'''
