...
heartbeat_ip=["192.168.29.151"]*10
...
try:	
	vehicle1= connect('udpin:192.168.29.151:14551',baud=115200, heartbeat_timeout=heartbeat_ip_timeout[0])
...
try:		
	vehicle2= connect('udpin:192.168.29.151:14552',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[1])
...
try:
	vehicle3= connect('udpin:192.168.29.151:14553',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[2])
...
try:		
	vehicle4= connect('udpin:192.168.29.151:14554',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[3])
...
try:		
	vehicle5= connect('udpin:192.168.29.151:14555',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[4])
...
try:
	vehicle6= connect('udpin:192.168.29.151:14556',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[5])
...
try:
	vehicle7= connect('udpin:192.168.29.151:14557',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[6])
...
try:
	vehicle8= connect('udpin:192.168.29.151:14558',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[7])
...
try:	
	vehicle9= connect('udpin:192.168.29.151:14559',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[8])
...
try:	
	vehicle10= connect('udpin:192.168.29.151:14560',baud=115200,heartbeat_timeout=heartbeat_ip_timeout[9])
...
slave_heal_ip=["192.168.29.151"]*len(vehicles)
			...
			for b in s.swarm:
				...
				while 1:
					...
					if(step==1):											
						...
						for i,b in enumerate(s.swarm):							
							...
							if (all_bot_reach_flag==True):								
								...
								with open(r'/home/dhaksha/Desktop/main/dce_swarm_nav/swarm_tasks-main/swarm_tasks/Examples/basic_tasks/pos.csv', 'a') as csvfile:
									...
									"""
									#path="goal_table"+","+str(multiple_goals.index(goal))	
									path=(str('table')+','+str(multiple_goals.index(goal)))	
									print("path%%%%%%%%%%",path)	
									#for i in range(20):
									#	print("I############",i,path)
						...
						#msg += ',path' + str(goal_path_csv_array[-1])
						...
						sent = graph_socket.sendto(str(msg).encode(), graph_server_address)
						"""
						print("path_csv_flag",path_csv_flag)
						if(path_csv_flag):
							print("^^^^^^^^^^^^^^")
							graph_socket.sendto(str(path).encode(), graph_server_address)
						"""
					#gui.update()
			...
			while 1:	
				...
				try:
					...
					for i,b in enumerate(s.swarm):									
						...
						if((area*10)>64) or (aggregate_flag):
							aggregate_flag=True
							break																		
						heartbeat[i]=vehicles[i].last_heartbeat
						#print( "heartbeat",heartbeat)
