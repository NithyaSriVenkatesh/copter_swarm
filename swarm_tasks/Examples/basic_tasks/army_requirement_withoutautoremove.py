...
# Bind the socket to the port
graph_server_address = ('192.168.6.219', 12009)  #receive from .....rx.py
...
#uav4_server_address = ('192.168.6.151', 12002)
#uav5 = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
#uav5_server_address = ('192.168.6.151', 12002)
...
try:
	...
	graph_socket.sendto(str("Drone 5 master received").encode(), graph_server_address)	
except:
	pass		
while True:
	try:
		...
		print("decoded_index",decoded_index)
		graph_socket.sendto(str("Drone 5"+decoded_index).encode(), graph_server_address)
...
# Bind the socket to the port
server_address = ('192.168.6.219', 12005)  #receive from .....rx.py

# Bind the socket to the port
remove_bot_server_address = ('192.168.6.219', 12001)  #receive from .....rx.py
...
vehicles=[]
pos_array=[]
heartbeat_ip=["192.168.6.151","192.168.6.152","192.168.6.153","192.168.6.154","192.168.6.151","192.168.6.156","192.168.6.157","192.168.6.158","192.168.6.159","192.168.6.160"]
...
def vehicle_collision_moniter_receive():	
        ...
        while 1:
        	...
        	if decoded_index.startswith("master"):
        		...
        		m,master_num=decoded_index.split("-")
        		print("m,master_num",m,master_num)
        		msg='Drone 5 master_num '+str(master_num)+' data received'
        		for l in range(2):
...
slave_heal_ip=["192.168.6.151"]*num_bots
...
server_address1 = ('192.168.6.219', 12010)
slave_heal_ip=["192.168.6.151"]*num_bots
...
removed_grid_path_array=[0]*num_bots
removed_grid_path_array_start_val=[0]*num_bots
...
while True:
	if(uav_home_pos!=[]):
		...
		for l in range(3):
			graph_socket.sendto(str("Drone 5 CONNECTED").encode(), graph_server_address)	
		break
	else:
		pass
#gui = viz.Gui(s)
			...
			while circle_formation_count<2500:
				...
				for i,b in enumerate(s.swarm):
					...
					if master_flag:
						...
						print("Drone 5 position",b.x,b.y)
						lat,lon = locatePosition.cartToGeo (origin, endDistance, current_position)
						#print ("Drone 5 lat,lon", lat,lon)
						if same_alt_flag:
							point1 = LocationGlobalRelative(lat,lon,same_height)
			...
			while 1:	
				...
				for i,b in enumerate(s.swarm):
					...
					if all(x >= int(num_lines) for x in grid_path_array) and removed_grid_path_length!=[] and not removed_grid_path_array_flag:
						...
						if len(removed_grid_path_length)==1:
							remaining_points = num_lines - removed_grid_path_length[0]
							points_per_drone = remaining_points // len(pos_array)
							extra_points = remaining_points % len(pos_array)
							print("points_per_drone",points_per_drone,extra_points)
							start_index = removed_grid_path_length[0] + 1
							print("start_index,extra_points",start_index,extra_points)
							for h in range(len(pos_array)):
								if h < extra_points:
									end_index = start_index + points_per_drone + 1
								else:
									end_index = start_index + points_per_drone
								print("end_index",end_index)
								if end_index > num_lines:
									end_index = num_lines
								print("end_index!!!",end_index)
							    # Store the range for the current drone
								removed_grid_path_array[h] = (start_index, end_index)
								removed_grid_path_array_start_val[h]=start_index
								removed_grid_filename[h]=removed_uav_grid[0]
							    # Update the starting index for the next drone
					...
					if(removed_grid_path_array_flag):
						goal_lat_lon = goal_lat_lon = read_specific_line(removed_grid_filename[i], removed_grid_path_array_start_val[i])						
					else:
					...
					if(dx<=2 and dy<=2):						
						...
						if grid_path_array[i]>=int(num_lines) and removed_grid_path_array_flag:
							...
							print("removed_grid_path_array_start_val!!!!!",i,removed_grid_path_array_start_val)
